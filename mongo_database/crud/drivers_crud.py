...
def create_item(main_collection: Collection, item: Drivers):
    item.version = 1
    main_collection.insert_one(item.model_dump())
    return item

def read_item(main_collection: Collection, name: str, team: str, race: str):
    if name:
        result = main_collection.find_one({"name": name})
    elif team:
        result = main_collection.find_one({"team_name": team})
    else:
        result = main_collection.find_one({"$or":[{"name": name},{"race_weekend":race}]})
    return result

def update_item(main_collection: Collection, cache_collection: Collection, name: str, team: str, item: Drivers, race: str):
    if name:
        find_item = main_collection.find_one({"name": name})
    elif team:
        find_item = main_collection.find_one({"team_name": team})
    else:

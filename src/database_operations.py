...
class InsertData:
    ...
    def _post_driver(self,item_dict,driver):
        try:
            get_existing_indx = self.data[f"{item_dict['year']}"][item_dict['raceLoc']][item_dict["session"]].index(item_dict[f"{driver}"])
            self.data[f"{item_dict['year']}"][item_dict['raceLoc']][item_dict["session"]][get_existing_indx] = ''
        except ValueError:
